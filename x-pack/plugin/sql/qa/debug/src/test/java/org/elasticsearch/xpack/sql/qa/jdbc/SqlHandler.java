...
class SqlHandler implements HttpHandler, AutoCloseable {
    ...
    protected static final Logger log = LogManager.getLogger(SqlHandler.class.getName());
    ...
    private FakeRestRequest createRequest(HttpExchange http) throws IOException {
        ...
        XContentType contentType = XContentType.fromMediaTypeOrFormat(
                headers.getOrDefault("content-type", singletonList(XContentType.JSON.mediaType())).get(0));
    ...
    protected void sendHttpResponse(HttpExchange http, RestResponse response) throws IOException {
        try {
            ...
            if (http.getResponseHeaders().isEmpty()) {
                ...
                headers.put("content-type", singletonList(response.contentType()));
